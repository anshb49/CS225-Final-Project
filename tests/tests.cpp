...
TEST_CASE("Verify BFS output string is accurate - Large Sample") {
	...
	std::string correct_string = "0 - 1000 Views: 297 ||  1000 - 100000 Views: 1411 ||  100001 - 50000 Views: 693 ||  50001 - 100000 Views: 126 ||  Over 100000 Views: 267 ||  Over 100000 Views: 2794 || ";
...
TEST_CASE("Verify that Dijkstra Algorithm output contains Correct Edge Weight - Large Sample") {
	GraphInitializer gc(data_vector, edgesVec);
	Graph outputGraph = gc.getGraph(); 
	GraphAlgo ga(outputGraph);
	std::map<string, std::vector<string>> feat_map = gc.getFeaturesMap();
	...
	vector<int> correct_vector;
	correct_vector.push_back(455941);
	correct_vector.push_back(2477);
	correct_vector.push_back(75542);
	correct_vector.push_back(3339);
	correct_vector.push_back(25714);

	for (int i = 0; i < (int)get<0>(djikstra_output).size() - 1; i++) {
		REQUIRE(outputGraph.getEdgeWeight(get<0>(djikstra_output).at(i), get<0>(djikstra_output).at(i+1)) == correct_vector[i]);
	}
}
...
TEST_CASE("Verify that A* Algorithm output contains Correct Edge Weight - Large Sample") {
	...
	correct_vector.push_back(14914146);
	correct_vector.push_back(5164);
